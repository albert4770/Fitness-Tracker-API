	...
	describe('Users', () => {
		...
		describe('createUser({ username, password })', () => {
			...
			it('Creates the user', async () => {
				expect(userToCreateAndUpdate.username).toBe(
					userCredentials.username
				);
      ...
      `);
		...
		describe('createActivity({ name, description })', () => {
			it('Creates and returns the new activity', async () => {
				...
				expect(createdActivity.description).toBe(
	...
	describe('Routines', () => {
		...
		describe('createRoutine', () => {
			it('creates and returns the new routine', async () => {
				routineToCreateAndUpdate = await createRoutine({
					...
					name: 'BodyWeight Day',
					goal:
						'Do workouts that can be done from home, no gym or weights required.'
				});
		...
		describe('updateRoutine', () => {
			...
			beforeAll(async () => {
				...
				queriedRoutine = await getRoutineById(
					routineToCreateAndUpdate.id
				);
			});
			...
			it('Updates the public status, name, or goal, as necessary', async () => {
				expect(routineToCreateAndUpdate.isPublic).toBe(
					queriedRoutine.isPublic
				);
